...
void BarrierRecorder::recordBarrierTransition(ResourceHandle handle, D3D12_RESOURCE_STATES newState)
{
    if (ResourceManager::it().getResourceState(handle) != newState)
    {
        ...
        m_recordedBarriers[m_numRecordedBarriers] = barrier;
        m_numRecordedBarriers++;
    }
}
...
{
    if (m_numRecordedBarriers > 0)
    {
        commandList->ResourceBarrier(static_cast<UINT>(m_numRecordedBarriers), m_recordedBarriers.data());
        m_numRecordedBarriers = 0;
    }
}
