...
TextureCube cubemap_tex : register( t4 );
...
float4 main_ps( float4 pos : SV_POSITION, float3 cam_pos : POSITION0, float3 world_pos : POSITION1, float3 normal : NORMAL0, float2 uvs : TEXCOORDS, float3 tangent : TANGENT, float3 bitangent : BITANGENT ) : SV_Target
{
    ...
    if ( length( sampled_N ) != 0.0 )
    {
        float3x3 TBN = float3x3( T, B, N );
        TBN = transpose( TBN ); // Transpose because float3x3() takes row vectors
